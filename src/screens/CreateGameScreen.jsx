        ...
        <>
            <Title>Players:</Title>
            {
                Object.entries(players)
                    .map((item, index) => {
                        const [id, name] = item;
                        return <PlayerInput key={id} index={index} user={{id, name}} setName={changePlayerName} deletePlayer={deletePlayer}/>
                    })
            }
            {playersLength < 4 && <Button onClick={addPlayer}>Add</Button>}
            <Button href={links.authorized.score}>Play</Button>
        </>
    )
}
...
const PlayerInput = ({index, user, setName, deletePlayer}) => {
    return (
        <div>
            <p>{index + 1}</p>
            <input placeholder={"Write your name"} value={user.name} onChange={setName(user.id)}/>
